...
void printdev(libusb_device* dev) {
	...
	libusb_open(dev, &handle);
        if (handle)
        {
            r = libusb_get_string_descriptor_ascii(handle, desc.iSerialNumber, str, sizeof(str)); //печать поля iSerialNumber дескриптора устройства
            if (r > 0) {
                printf("%s\n", str);
            }
            else {
            ...
            r = libusb_get_string_descriptor_ascii(handle, desc.iProduct, str, sizeof(str)); //печать поля iProduct дескриптора устройства
            if (r > 0) {
                printf("%s\n", str);
            }
            else {
	    ...
	    r = libusb_get_string_descriptor_ascii(handle, desc.iManufacturer, str, sizeof(str)); //печать поля iManufacturer дескриптора устройства
            if (r > 0) {
                printf("%s\n", str);
            }
            else {
